...
void SerialPort::setSignals(int rxPin, int txPin, int rtsPin, int ctsPin, int dtrPin, int dsrPin, int dcdPin, int riPin)
{
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  esp_rom_gpio_pad_select_gpio((gpio_num_t) m_rxPin);
#else
  PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_rxPin], PIN_FUNC_GPIO);
#endif
  gpio_set_direction(m_rxPin, GPIO_MODE_INPUT);

  // TX (out)
  m_txPin = int2gpio(txPin);
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
  esp_rom_gpio_pad_select_gpio((gpio_num_t) m_txPin);
#else
  PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_txPin], PIN_FUNC_GPIO);
#endif
  ...
  if (m_rtsPin != GPIO_UNUSED) {
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    esp_rom_gpio_pad_select_gpio((gpio_num_t) m_rtsPin);
#else
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_rtsPin], PIN_FUNC_GPIO);
#endif
    gpio_set_direction(m_rtsPin, GPIO_MODE_OUTPUT);
  ...
  if (m_dtrPin != GPIO_UNUSED) {
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    esp_rom_gpio_pad_select_gpio((gpio_num_t) m_dtrPin);
#else
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_dtrPin], PIN_FUNC_GPIO);
#endif
    gpio_set_direction(m_dtrPin, GPIO_MODE_OUTPUT);
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    esp_rom_gpio_pad_select_gpio((gpio_num_t) m_ctsPin);
#else
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_ctsPin], PIN_FUNC_GPIO);
#endif
    gpio_set_direction(m_ctsPin, GPIO_MODE_INPUT);
  }
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    esp_rom_gpio_pad_select_gpio((gpio_num_t) m_dsrPin);
#else
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_dsrPin], PIN_FUNC_GPIO);
#endif
    gpio_set_direction(m_dsrPin, GPIO_MODE_INPUT);
  }
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    esp_rom_gpio_pad_select_gpio((gpio_num_t) m_dcdPin);
#else
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_dcdPin], PIN_FUNC_GPIO);
#endif
    gpio_set_direction(m_dcdPin, GPIO_MODE_INPUT);
  }
...
#if ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(5, 0, 0)
    esp_rom_gpio_pad_select_gpio((gpio_num_t) m_riPin);
#else
    PIN_FUNC_SELECT(GPIO_PIN_MUX_REG[m_riPin], PIN_FUNC_GPIO);
#endif
    gpio_set_direction(m_riPin, GPIO_MODE_INPUT);
  }
}
...
{
  // Get Advanced Peripheral Bus (APB) frequency
  rtc_cpu_freq_config_t conf;
  rtc_clk_cpu_freq_get_config(&conf);

  // On ESP32, the APB bus frequency is tied to the CPU frequency.
  // If the CPU frequency is 80MHz or higher, the APB frequency is 
  // fixed at 80MHz to maintain stable peripheral timing (VGA/I2S).
  // If the CPU frequency is lower than 80MHz, the APB frequency 
  // drops to match the CPU frequency.

  //uint32_t apb_freq = (conf.freq_mhz >= 80) ? 80000000 : (conf.source_freq_mhz * 80000000 / conf.div);
  uint32_t apb_freq = (conf.freq_mhz >= 80) ? 80000000 : (conf.freq_mhz * 1000000);

  uint32_t clk_div = (apb_freq << 4) / value;
