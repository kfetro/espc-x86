...
#include "drivers/signal/gpio_stream.h"
#include "drivers/video/vga_palette.h"
...
void VGAPalettedController::calculateAvailableCyclesForDrawings()
{
  ...
  m_primitiveExecTimeoutCycles = conf.freq_mhz * availtime_us;  // at 240Mhz, there are 240 cycles every microsecond
}
...
void VGAPalettedController::RGB222_to_HSV(int R, int G, int B, double * h, double * s, double * v)
{
  double r = R / 3.0;
  double g = G / 3.0;
  double b = B / 3.0;
