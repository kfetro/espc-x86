...
// Video Memory
#define TGA_VRAM_SIZE 32768

#define TGA_VRAM_BASE  0xB8000
#define TGA_VRAM_LIMIT 0xBFFFF

// IO Ports
#define TGA_PORT_CRTCIDX     0x03D4
#define TGA_PORT_CRTCDATA    0x03D5
#define TGA_PORT_MODECTRL    0x03D8
#define TGA_PORT_COLORSEL    0x03D9
#define TGA_PORT_STATUS      0x03DA
#define TGA_PORT_EXTRAMPAGE  0x03DD
#define TGA_PORT_ARRAYDATA   0x03DE
#define TGA_PORT_CRTCPUPAGE  0x03DF

// CRTC (Cathode Ray Tube Controller) Registers
#define TGA_CRTC_CURSORSTART   0x0A
#define TGA_CRTC_CURSOREND     0x0B
#define TGA_CRTC_STARTADDR_HI  0x0C
#define TGA_CRTC_STARTADDR_LO  0x0D
#define TGA_CRTC_CURSORPOS_HI  0x0E
#define TGA_CRTC_CURSORPOS_LO  0x0F

// Mode Control Register
#define TGA_MC_TEXT80COLS  0x01
#define TGA_MC_GRAPHICS    0x02
#define TGA_MC_MONOCHROME  0x04
#define TGA_MC_ENABLED     0x08
#define TGA_MC_HIGHRES     0x10
#define TGA_MC_BIT7BLINK   0x20

// Color Select Register
#define TGA_CS_COLOR_MASK     0x0F
#define TGA_CS_HIGHINTENSITY  0x10
#define TGA_CS_PALETTESEL     0x20

// Default value in CGA composite
#define TGA_DEFAULT_COLORSELECT  0x30
// Default value in CGA RGB (IBM 5153)
//#define TGA_DEFAULT_COLORSELECT  0x00

// Tandy Video Control Register
#define TGA_VC_ENABLEGFX  0x01
#define TGA_VC_LOWRES     0x02
...
// Tandy video card emulation
class Tandy : public VideoAdapter,
...
public:
  ...
  size_t vramSize() override { return TGA_VRAM_SIZE; }
...
private:
  ...
  // Tandy
  uint8_t m_extMemPage; // Extended RAM Page Register
  uint8_t m_arrayData;

  uint8_t m_crtPage;
  uint8_t m_cpuPage;
  uint8_t m_addressMode;
  ...
  inline bool isGraphicsMode()   const { return (m_modeControl & TGA_MC_GRAPHICS) != 0; }
  inline bool isHighResolution() const { return (m_modeControl & TGA_MC_HIGHRES) != 0; }
  inline bool isVideoEnabled()   const { return (m_modeControl & TGA_MC_ENABLED) != 0; }
  inline bool isBit7Blinking()   const { return (m_modeControl & TGA_MC_BIT7BLINK) != 0; }

  // Color Select
  inline uint8_t paletteIndex() const {
    const uint8_t paletteAltern = (m_colorSelect & TGA_CS_PALETTESEL)    ? 2 : 0;
    const uint8_t highIntensity = (m_colorSelect & TGA_CS_HIGHINTENSITY) ? 1 : 0;
  ...
  inline uint8_t colorSelect() const { return m_colorSelect & TGA_CS_COLOR_MASK; }

  // CGA
  void writePixel320x200x4(uint32_t x, uint32_t y, uint8_t value, bool xored);
  void writePixel640x200x2(uint32_t x, uint32_t y, bool on);
  // Tandy
  void writePixel160x200x16(uint32_t x, uint32_t y, uint8_t color);
  void writePixel320x200x16(uint32_t x, uint32_t y, uint8_t color);
  void writePixel640x200x4(uint32_t x, uint32_t y, uint8_t color);

  uint8_t readPixel160x200x16(uint32_t x, uint32_t y) const;
  uint8_t readPixel320x200x16(uint32_t x, uint32_t y) const;
  uint8_t readPixel640x200x4(uint32_t x, uint32_t y) const;
